...
- [~] Live snapshot/API contract for office, committee, chat, message, widget, portfolio, strategy, research, risk, and system surfaces. Evidence: `ai-office-ui/src/api/live.ts`, broad `/api/snapshot`, and scoped `/api/office/snapshot`, `/api/system-health/snapshot`, and `/api/mission-control/snapshot`. Portfolio, client, research, idea, trading, quant, risk, and report workspaces still require scoped contracts.
...
- [~] Command Center shell extraction from monolithic `App.tsx` with no behavior loss. Verified 2026-07-13: Live Office, System Health, and Mission Control now mount as independent workspace components. Direct System Health and Mission Control routing suppresses both the broad Command Center snapshot lifecycle and its stale right rail. Remaining Command Center workspace modules still need extraction. Evidence: [[2026-07-13-system-health-v2-and-docker-runtime-recovery]] and [[2026-07-13-mission-control-v2-scoped-workspace]].
...
- [~] Focused workspace snapshot profile and payload budget. Verified 2026-07-13: bounded `/api/snapshot` remains about 7.6 MB/9.7 s; `/api/office/snapshot` is 242 KB/105 ms; `/api/system-health/snapshot` is about 83 KB/0.12-0.75 s with 209 live rows across 16 bounded queries; `/api/mission-control/snapshot` is about 204 KB/0.27 s with 167 live rows across 14 bounded queries. Live Office, System Health, and Mission Control no longer require the broad poll, while the remaining Command Center workspaces still need scoped contracts. Evidence: [[2026-07-13-system-health-v2-and-docker-runtime-recovery]] and [[2026-07-13-mission-control-v2-scoped-workspace]].
