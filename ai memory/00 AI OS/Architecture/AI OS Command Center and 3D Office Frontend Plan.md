...
- The Command Center and Live Office world toggle is live. Legacy Command Center workspaces retain `/api/snapshot`; Mission Control and System Health use scoped read models, while Live Office uses `/api/office/snapshot` and mounts without starting the broad Command Center snapshot lifecycle.
...
- Performance measurement: bounded `/api/snapshot` took 9.20 seconds and returned 7.40 MB on 2026-07-10. The delivered `/api/office/snapshot` took 105 ms and returned 242 KB; `/api/system-health/snapshot` is about 83 KB; `/api/mission-control/snapshot` returned 204 KB in 0.27 seconds. Next: scoped Portfolio, Client, Research, Ideas, Trading, Quant, Risk, and Reports reads while keeping deep evidence available on demand.
