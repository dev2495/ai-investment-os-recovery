...
- Performance measurement: bounded `/api/snapshot` took 9.20 seconds and returned 7.40 MB. Reports now reads 605 KB in 0.23 seconds; every route is scoped. Deep evidence is loaded only when opened. Next: stale-data/error boundaries, remaining domain-specific drill-downs, dead-code removal, and production gates.
