...
- The Command Center and Live Office world toggle is live. Reports is now the only workspace retaining `/api/snapshot`; every other workspace has a scoped read model.
...
- Performance measurement: bounded `/api/snapshot` took 9.20 seconds and returned 7.40 MB. Trading/Quant/Risk now reads 228 KB in 0.13 seconds warm. Next: Reports, then deep evidence/action and production gates.
