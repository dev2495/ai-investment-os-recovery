...
Verified foundation (updated 2026-07-13): the live Postgres operating surface returns 36 agent profiles, 10 office rooms, 24 live activity records, 11 departments, 77 skills, and 10 strategy templates without seed fallback. Live Office is independently mounted on `/api/office/snapshot` at 242 KB/105 ms. System Health, Mission Control, Portfolio Office, and Client Folios are independently mounted Command Center workspaces; none starts the 7.40 MB broad snapshot or stale right rail. Mission Control exposes Charlie chat, durable handoffs, task/inbox queues, approval visibility, execution/provider gates, widget materialization, worker launch, and source freshness. Portfolio Office and Client Folios expose real clients/accounts/holdings, multi-book positions and exposure, portfolio intelligence, readiness gaps, reconciliations, and approval-gated holding staging. Its shared scoped read measured 196 KB/0.30 s for 313 rows across 15 queries. The first office evidence drawer remains live for mailbox work: message `#61` resolves through `/api/evidence/agent-message/61` to task `#294`, inbox item `#379`, and the worker output note path. Phase 8 remains partial until the remaining workspaces are scoped, thesis/attribution/client-onboarding controls are complete, broader decision evidence and task arrows are complete, and accessibility/performance gates pass.
