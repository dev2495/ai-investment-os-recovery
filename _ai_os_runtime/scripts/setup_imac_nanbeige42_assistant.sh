...
INSERT INTO core.connector_health_checks (
    ...
    'model_endpoint', 'ollama_nanbeige42_3b_q4_imac',
    'Nanbeige4.2 runtime compatibility', 'runtime_probe', :'health_status',
...
if ! pull_output="$("${OLLAMA_BIN}" pull "${MODEL}" 2>&1)"; then
  ...
  echo "Nanbeige4.2 remains disabled because the configured Ollama manifest is unavailable." >&2
  exit 4
fi
...
if ! python3 "${RUNTIME_ROOT}/scripts/run_local_model_evals.py" \
  ...
  echo "Nanbeige4.2 remains disabled because conversation_v1 did not pass." >&2
  exit 5
fi
