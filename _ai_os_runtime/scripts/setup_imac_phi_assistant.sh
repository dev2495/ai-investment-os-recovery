...
[[ -f "${RUNTIME_ROOT}/postgres/init/161_phi4_mini_imac_basic_assistant.sql" ]] || {
  echo "Phi-4 Mini model migration is missing from ${RUNTIME_ROOT}." >&2
  exit 3
}
...
"${DOCKER_BIN}" exec -i ai_os_postgres psql -U ai_os -d ai_os -v ON_ERROR_STOP=1 \
  < "${RUNTIME_ROOT}/postgres/init/161_phi4_mini_imac_basic_assistant.sql"
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE model_name='phi4-mini:3.8b'
      AND eval_suite='conversation_v1'
      AND promotion_status='approved'
      AND coalesce(last_eval_score, 0) >= 0.8
  ) THEN
    RAISE EXCEPTION 'Phi-4 Mini activation refused: conversation_v1 promotion evidence is missing';
  END IF;
END $$;
