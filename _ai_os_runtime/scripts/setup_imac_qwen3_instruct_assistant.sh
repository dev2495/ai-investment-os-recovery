...
[[ -f "${RUNTIME_ROOT}/postgres/init/163_qwen3_4b_instruct_imac_assistant.sql" ]] || {
...
"${DOCKER_BIN}" exec -i ai_os_postgres psql -U ai_os -d ai_os -v ON_ERROR_STOP=1 \
  < "${RUNTIME_ROOT}/postgres/init/163_qwen3_4b_instruct_imac_assistant.sql"
...
SET status='disabled', health_status='eval_failed', last_checked_at=now(),
    ...
    updated_at=now()
WHERE endpoint_key='ollama_qwen3_4b_instruct_imac';
SQL
  echo "Qwen3 4B Instruct remains disabled because conversation_v1 did not pass." >&2
  exit 1
fi
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE model_name='qwen3:4b-instruct'
      AND eval_suite='conversation_v1'
      AND promotion_status='approved'
      AND coalesce(last_eval_score, 0) >= 0.8
  ) THEN
    RAISE EXCEPTION 'Qwen3 Instruct activation refused: conversation_v1 promotion evidence is missing';
  END IF;
END $$;
...
echo "Qwen3 4B Instruct is evaluated, approved, and active as Charlie's private fallback."
