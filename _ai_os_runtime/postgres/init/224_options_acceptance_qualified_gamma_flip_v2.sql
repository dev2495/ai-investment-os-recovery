...
DECLARE
    v_run_id BIGINT;
    v_gate_version CONSTANT TEXT := 'institutional-options-acceptance-v2';
        ...
        SELECT * FROM (VALUES
            ...
            ('exposure_estimates','GEX DEX Vanna Charm And Gamma Flip',
             (SELECT count(DISTINCT estimate.metric_name)::numeric FROM trading.option_exposure_estimates estimate
              ...
              WHERE estimate.metric_name IN ('gex','dex','vanna','charm','gamma_flip')
                AND (
                    (estimate.metric_name<>'gamma_flip'
                     AND estimate.quality_status IN ('passed','warning')
                     AND estimate.metric_value IS NOT NULL)
                    OR
                    (estimate.metric_name='gamma_flip' AND (
                        (estimate.quality_status IN ('passed','warning')
                         AND estimate.metric_value IS NOT NULL)
                        OR
                        (estimate.quality_status='not_computable'
                         AND estimate.metric_value IS NULL
                         AND estimate.assumptions->>'crossing_found'='false'
                         AND estimate.assumptions->>'result_interpretation'='no_crossing_within_tested_grid'
                         AND jsonb_array_length(estimate.spot_grid)>1)
                    ))
                )),5::numeric,'gte',
             jsonb_build_object(
                 'requires','GEX, DEX, vanna, charm, and a transparent gamma-flip grid result',
                 'gamma_flip_null_allowed_only_when','qualified grid has no crossing',
                 'black_box_signal_allowed',false
             )),
...
COMMENT ON FUNCTION trading.run_option_acceptance_gates IS
    'Evaluates source-backed institutional options acceptance v2. A null gamma-flip is accepted only as an explicit no-crossing result on a populated qualified grid; no order, approval, market row, or synthetic evidence is created.';
