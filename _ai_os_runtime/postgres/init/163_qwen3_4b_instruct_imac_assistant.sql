...
SET promotion_status='rejected',
    notes='Rejected for Charlie: the official qwen3:4b alias resolves to a thinking checkpoint and leaked internal analysis on the production conversation surface.',
    updated_at=now()
WHERE model_name='qwen3:4b';

UPDATE agent.model_endpoints
SET status='disabled', health_status='failed',
    last_error='Rejected: production conversation surface leaked internal analysis.', updated_at=now()
...
INSERT INTO agent.local_model_registry (
    ...
    'qwen3:4b-instruct', 'ollama', 'light', ARRAY['imac'], 8192,
    ...
    'Always-on iMac non-thinking instruct assistant. It may converse, intake tasks, select allowlisted tools, and summarize supplied evidence. It has no calculation, research-authority, capital, approval, broker, or execution authority.',
    '[{"source":"https://ollama.com/library/qwen3:4b-instruct","model_tag":"qwen3:4b-instruct","expected_disk_gb":2.5,"license":"Apache-2.0","eval_required":"conversation_v1","raw_prompt_stored":false,"live_execution_allowed":false}]'::jsonb
)
...
INSERT INTO agent.model_routes (
    ...
    'ollama', 'qwen3:4b-instruct', 'openrouter', 'z-ai/glm-4.7-flash', 'local',
...
INSERT INTO agent.model_endpoints (
    ...
    'ollama_qwen3_4b_instruct_imac', 'iMac Qwen3 4B Instruct always-on assistant',
    'ollama', 'qwen3:4b-instruct', 'imac_basic_assistant_qwen3', 'local',
...
WHERE endpoint.endpoint_key='ollama_qwen3_4b_instruct_imac'
  AND registry.model_name='qwen3:4b-instruct'
