...
function InvestmentBrief({ selected, business, moat, opinions, risks, filings, models, financialHistory, productionRatios, validationChecks }: {
  selected: LiveRow; business?: LiveRow; moat?: LiveRow; opinions: Map<string, LiveRow>; risks: LiveRow[]; filings: LiveRow[]; models: LiveRow[];
  financialHistory: LiveRow[]; productionRatios: LiveRow[]; validationChecks: LiveRow[];
}) {
  ...
  const debt = latestYear ? (factValue(latestYear, "current_borrowings") || 0) + (factValue(latestYear, "non_current_borrowings") || 0) : null;
  const cash = latestYear ? (factValue(latestYear, "cash_and_cash_equivalents") || 0) + (factValue(latestYear, "other_bank_balances") || 0) : null;
  ...
  const priceMatch = text(valuationPack, "summary").match(/(?:INR|₹)\s*([\d,]+(?:\.\d+)?)/i);
  ...
  return (
    <main className="thesis-workspace">
      ...
      <details className="ltw-research-operations ltw-frontstage-drawer" open={researchOpsOpen} onToggle={(event) => setResearchOpsOpen(event.currentTarget.open)}>
        ...
        <ResearchCaseWorkspace selected={selected} cases={data.research_cases} agents={data.research_case_agents} workItems={data.research_case_work_items} evidence={data.research_case_evidence} events={data.research_case_events} preflights={data.model_run_preflights} modelRuns={data.research_case_model_runs} onRefresh={() => void query.refetch()} />
      </details>
      ...
      <div className="ltw-layout">
        ...
        <div className="ltw-document">
          ...
          <ResearchSection id="financials" eyebrow="04 · investor decision workspace" title="Financial Quality" state={data.financial_facts.length ? "machine extracted" : "missing"} source={data.financial_facts[0] || financeSection}>
