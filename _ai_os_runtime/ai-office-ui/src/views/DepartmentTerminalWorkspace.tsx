  ...
  return <section className="committee-operator-grid">
    ...
    <section className="terminal-pane committee-session-console">
      ...
      {selectedPacket ? <>
        ...
        <div aria-label="Committee member positions" className="committee-position-preview" role="region" tabIndex={0}>{packetPositions.map((row)=><article key={text(row,"id")}><div><strong>{text(row,"display_title")} · {text(row,"stance").replace(/_/g," ")}</strong><p>{text(row,"recommendation").replace(/_/g," ")}</p></div><span>{text(row,"confidence")}%</span></article>)}{!packetPositions.length?<p>Independent position assignments are waiting in member inboxes.</p>:null}</div>
