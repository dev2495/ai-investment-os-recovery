  ...
  return <div className="integration-gateway-workspace">
    ...
    <section className="dashboard-grid">
      ...
      <Panel className="span-12" icon={<DatabaseZap size={17}/>} title="Strategy Data Readiness" action={<button className="mini-action-button" disabled={Boolean(busy)} onClick={()=>void act("legacy-market",()=>runGatewayJob("legacy_market_data_manual_ingestion"),"Legacy market data revalidated and imported idempotently.")} type="button"><Play size={13}/>{busy==="legacy-market"?"Importing":"Revalidate sources"}</button>}>
        ...
        <div className="gateway-market-grid" role="region" tabIndex={0} aria-label="Market data bias controls">
          {(snapshot?.market_bias_readiness ?? []).map((row)=><article key={value(row,"control_key")}><div><span>{value(row,"control_key").replace(/_/g," ")}</span><strong>{Number(value(row,"observed_rows","0")).toLocaleString()} observed</strong><small>{value(row,"mapped_rows","0")} mapped · {value(row,"verified_rows","0")} verified · {value(row,"applied_rows","0")} applied</small></div><div><StatusPill status={value(row,"readiness_status")}/></div><p>{value(row,"next_required_action")}</p></article>)}
        </div>
