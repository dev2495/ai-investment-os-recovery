...
test("Client Folios exposes governed lifecycle and live reconciliation controls", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { level: 2, name: "NAV & Cash Evidence" })).toBeVisible();
  await expect(page.getByRole("heading", { level: 2, name: "Performance & Benchmark" })).toBeVisible();
  await expect(page.getByRole("heading", { level: 2, name: "FIFO Tax-Lot Control" })).toBeVisible();
  await expect(page.getByRole("heading", { level: 2, name: "Realized Attribution" })).toBeVisible();
  await expect(page.getByRole("heading", { level: 2, name: "Stage Cash Evidence" })).toBeVisible();
  await expect(page.getByRole("heading", { level: 2, name: "Cash Approval Queue" })).toBeVisible();
  await expect(page.getByRole("heading", { level: 2, name: "Client Report Delivery" })).toBeVisible();
  ...
  await expect(page.getByRole("button", { name: "Stage cash entry" })).toBeDisabled();
  await expect(page.getByRole("button", { name: "Recalculate" })).toBeVisible();
  await expect(page.getByText("opening_cash_balance", { exact: false }).first()).toBeVisible();
  await expect(page.getByText("transaction_history_for_current_positions", { exact: false }).first()).toBeVisible();
